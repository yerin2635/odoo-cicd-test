...
var KanbanRecord = Widget.extend({
    ...
    _render: function () {
        ...
        if(this.modelName != "pchome.dashboard" &&
            this.modelName != "pchome.process.monitoring") {
            this.$el.addClass('o_kanban_record').attr("tabindex", 0);
        }
        if(this.modelName == 'pchome.process.monitoring' &&
            this.__parentedParent.$el.parent().attr('class') != "contentWrap") {
            this.__parentedParent.$el.wrap('<div class="contentWrap"></div>');
        }
